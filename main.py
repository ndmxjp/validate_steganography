...
if True:
    ...
    for embed_rate in np.arange(1, 10 +1) / 10:
        ...
        features = []
        labels   = []
        with open('./f5_multiplication-reference-feature/f5_reference_all_mult_0.0.csv', 'r') as f:
            for line in f:
                feature = np.array([float(e) for e in line.split(',')])
                features.append(feature)
                labels.append(0)

        with open('./f5_multiplication-reference-feature/f5_reference_all_mult_%.1f.csv' % embed_rate, 'r') as f:
            for line in f:
                feature = np.array([float(e) for e in line.split(',')])
                features.append(feature)
                labels.append(1)
